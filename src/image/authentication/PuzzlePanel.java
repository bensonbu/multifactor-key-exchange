...
public class PuzzlePanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    JFrame jf = null;
    public static boolean solvedStatus = false;
...
//  g2.draw(repaintRect2);
    ...
    protected void stopDragging(MouseEvent e) {
        ...
        /*
         ...
         ImageIcon imgic = new ImageIcon("src/org/game/Juzzle/images/"+JuzzlePanel.splittedFilename); 
 
         int width = imgic.getIconWidth();
         int hght = imgic.getIconHeight();
         jf.add(new JLabel(imgic), BorderLayout.CENTER);
         jf.setSize(width, hght);
         jf.setVisible(true);
         ...
         */
        // if solved the whole image, show the message one time
        if (!endMessagePrinted && puzzlePartList.getSolvedPartsCount() >= 4) {
            controlPanel.stopCounter();

            solvedStatus = true;
            SecureKey.solved = true;
